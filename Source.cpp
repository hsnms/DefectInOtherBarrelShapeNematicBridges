//Source.cpp helps to find the specific equilibrium nematic defect structures in a barrel-shaped nematic bridge given specific parameters including the Frank constant ratio and aspect ratio.
 ...
 for(j=1;j<=(int)(n*H-H+1);j++)
  {
	...
	u[qq[(int)(n*H-H+1)]][(int)(n*H-H+1)]=0.0;//the right-up point points up

	for (j=2;j<(int)(n*H-H+1);j=j+1)
		{
		 
		  u[qq[j]][j]=/*atan(2.0/((double)((-qq[j+1]+qq[j-1]))))*/atan(2.0/((double)((n-1)*(-q[j+1]+q[j-1]))));//directors on the curved boundary
	...
	mgfas5new(u,n,ncycle,m,b,qq,x,y, R0,K, H);//solve PDE, inside region and lower boundary
		f=dmatrix2new(1,(int)(n*R0-R0+1),1,(int)(n*H-H+1));
		
		nrfunc4new(f,u,n,qq,b,R0,K,H);//integrand, almost the whole region
	
n1=1;
 z[1]=integratnew(f, n1,(int)(n*R0-R0+1),n1,(int)(n*H-H+1),n, H, R0)-integratnew(f, n1,n1+b,n1,b+1,n, H, R0)-integratnew(f, qq[(int)(n*H-H+1)-b]-b,(int)(n*R0-R0+1),(int)(n*H-H+1)-b,(int)(n*H-H+1),n, H, R0);//total free energy
 zero=Stat3new(u,n,b,R0,K, H,qq);//error, inside region and lower boundary
	result=z[1];
	for (j=1;j<=(int)(n*H-H+1);j++)
		for (i=1;i<=(int)(n*R0-R0+1);i++)
			v[i][j]=u[i][j];
